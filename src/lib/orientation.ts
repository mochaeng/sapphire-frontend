/*
All code in this file was sourced from: https://github.com/mooyoul/get-orientation
*/
...
export async function getOrientation(input: ArrayBuffer | File | Blob) {
    ...
    if (head === 0xffd8) {
      ...
      do {
        ...
        if (marker === 0xffe1) {
          ...
          const isEXIF =
            segmentHead.getUint16(2) === 0x4578 &&
      ...
      (head === 0x4949 && tail === 0x2a00) ||
      (head === 0x4d4d && tail === 0x002a)
    ) {
  ...
  while (fieldCount-- > 0) {
    ...
    if (tagId === 0x112) {
      // Orientation Tag
      const value = new DataView(bufField.slice(8, 12)).getUint16(
        0,
        isLittleEndian,
      );
